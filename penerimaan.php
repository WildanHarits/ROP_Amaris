...
if(isset($_POST['simpan'])){
      ...
      while($row = mysqli_fetch_assoc($result)){
        ...
        //echo $_POST['tgl'];
        $query="insert into transaksi values('biasa','".$_POST['tgl']."','".$row['id_barang']."','".$row['nama_barang']."','".$row['nama_kategori']."','".$row['satuan']."','".$jumlah."','".$row['harga_satuan']."','".$total."','1')";
        //$conn->query($query);
        if ($conn->query($query) === FALSE) {
          $query="update transaksi set nama_barang='".$row['nama_barang']."',nama_kategori='".$row['nama_kategori']."',satuan='".$row['satuan']."',jumlah='".$jumlah."' ,harga_satuan='".$row['harga_satuan']."',total='".$total."', status='1' where tgl='".$_POST['tgl']."' and id_barang='".$row['id_barang']."' and event='biasa'";
          //echo $query;
          $conn->query($query);
      ...
      header("Location:printpenerimaan.php?tgl=".$_POST['tgl']);
    }
...
<div class="wrapper">
      ...
      <ul class="sidebar-menu" data-widget="tree">
        ...
        <li class="active treeview">
          ...
          <ul class="treeview-menu">
            ...
            <li class="active"><a href="penerimaan.php"><i class="fa fa-circle-o"></i> Penerimaan</a></li>
          </ul>
        </li>
        <li>
  ...
  <div class="content-wrapper">
    ...
    $sql = "select distinct tgl from transaksi where status='0' order by tgl desc";
    $result = mysqli_query($conn, $sql);
    while($row = mysqli_fetch_assoc($result)){
      $tgl=$row['tgl'];
    }
  ?>
  <form method="POST">
    Tanggal : <input type="date" name="date" id="date" onchange=cektgl() value="<?php echo $tgl ?>" required>
  <input type="button" onclick=cari() value="cari">
  
  <input type="text" name="tgl" id="tgl" value="<?php echo $tgl ?>" hidden>
...
<div id="tampildata">
 ...
 <table id="example1" class="table table-bordered table-hover">
    ...
    <?php
    $sql = "select * from transaksi where tgl ='".$tgl."'";
    $result = mysqli_query($conn, $sql);
    while($row = mysqli_fetch_assoc($result)){
      if($row['jumlah']!='0'){
        echo "<tr>";
        echo "<td><input name='qty_".$row['id_barang']."' type='text' id='qty_".$row['id_barang']."' onkeyup=cekharga('".$row['id_barang']."','".$row['harga_satuan']."') value='".$row['jumlah']."'>";
        echo "</td>";
        echo "<td>".$row['satuan'];
        echo "</td>";
