...
if(isset($_POST['simpan'])){		
	...
	while($row = mysqli_fetch_assoc($result)){
		if($_POST['b-pemesanan-'.$row['id_bahan']]==""){
			$_POST['b-pemesanan-'.$row['id_bahan']] = 0;
			}
		if($_POST['b-penyimpanan-'.$row['id_bahan']]==""){
			$_POST['b-penyimpanan-'.$row['id_bahan']] = 0;
			}
		$query="insert into eoq values('".$_POST['bulan']."','".$_POST['tahun']."','".$row['id_bahan']."','".$_POST['b-pemesanan-'.$row['id_bahan']]."','".$_POST['b-penyimpanan-'.$row['id_bahan']]."')";
		if ($conn->query($query) === FALSE){
			$query="update eoq set b_pemesanan = '".$_POST['b-pemesanan-'.$row['id_bahan']]."', b_penyimpanan = '".$_POST['b-penyimpanan-'.$row['id_bahan']]."' where bulan = '".$_POST['bulan']."' and tahun='".$_POST['tahun']."' and id_bahan='".$row['id_bahan']."'";
			$conn->query($query);
			}
		}	
	}
      ...
      <ul class="sidebar-menu" data-widget="tree">
        ...
        <li class="treeview">
          ...
          <ul class="treeview-menu">
            <li><a href="index.php"><i class="fa fa-circle-o"></i> Data bahan baku</a></li>
        ...
        <li class="active treeview">
          ...
          <ul class="treeview-menu">
            <li class="active"><a href="eoq.php"><i class="fa fa-circle-o"></i> EOQ</a></li>
            ...
            <div class="box-body">
              <form method="post">
				...
				<table id="example1" class="table table-bordered table-hover">
					...
					if(isset($_POST['cek'])) {
						$aaa=1;
						$sql = "select * from stok_bahan a join eoq b on (a.id_bahan=b.id_bahan) join barang c on (a.id_bahan=c.id_barang) join biaya d on (c.nama_kategori=d.nama_kategori) where bulan='".$_POST['bulan']."' and tahun='".$_POST['tahun']."'";
					}else{
						$aaa=2;
						//$sql = "select * from stok_bahan a join barang c on (a.id_bahan=c.id_barang) join biaya d on (c.nama_kategori=d.nama_kategori)";
						/*$bulann='';
						$tahunn='';
						$sql = "select distinct tahun from eoq order by tahun desc limit 1";	
						$result = mysqli_query($conn, $sql);
						while($row = mysqli_fetch_assoc($result)){
							$sql2 = "select distinct bulan from eoq where tahun='".$row['tahun']."' order by bulan desc limit 1";	
					...
					while($row = mysqli_fetch_assoc($result)){
						...
						echo "<td>".$row['nama_bahan']."</td><td><input type='text' name='b-pemesanan-".$row['id_bahan']."' value='";
						if($aaa==1){
							echo $row['b_pemesanan'];
						}else if($aaa==2){
							echo $row['biaya_pemesanan'];
						}
						echo "'></td><td><input type='text' name='b-penyimpanan-".$row['id_bahan']."' value='";
						if($aaa==1){
							echo $row['b_penyimpanan'];
						}else if($aaa==2){
							echo $row['biaya_penyimpanan'];
						}
						echo "'></td>";
						...
						if($aaa==1){
							if($row['b_penyimpanan']!='0'){
								$sql2 = "select sum(jumlah) a from transaksi_bahan where date_format(tgl,'%c') = '".$_POST['bulan']."' and date_format(tgl,'%Y') = '".$_POST['tahun']."' and id_bahan='".$row['id_bahan']."'";
								//echo $sql2;	
								$result2 = mysqli_query($conn, $sql2);
								while($row2 = mysqli_fetch_assoc($result2)){
									$hasil = sqrt((2*$row2['a']*$row['b_pemesanan'])/$row['b_penyimpanan']);
								}
